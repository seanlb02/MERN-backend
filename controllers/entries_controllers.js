...
// middleware function to post a new entry to the database
    ...
    const getDailyTag = async function (req, res, next) {
        ...
        const {username} = req.params.username;
        try {
            const tag = await EntriesModel.find({username: username, timestamp: {$gte:pastDay}}, 'tags -_id').select('-_id')
            res.send(tag);
            next();
        }
        catch (err) {
    ...
    const getUserDayTags = async function (req, res, next) {
        const currentDateTime = new Date();
        const pastDay = currentDateTime.setDate(currentDateTime.getDate() - 1)
        const {username} = req.params.username;
        
        try {
            const tag = await EntriesModel.find({username: req.params.tracked, timestamp: {$gte:pastDay}}, 'tags -_id').select('-_id')
            res.send(tag);
            next();
        }
        catch (err) {
